...
指标:
  HitRate@K   top-K 来源中是否存在与期望答案匹配的分片 (漏检率 = 1 - HitRate)
  MRR@K       首个匹配来源倒数排名的均值
...
def write_reports(result: dict) -> tuple[Path, Path]:
    ...
    for dom, d in s["by_domain"].items():
        lines.append(f"| {dom} | {d['cases']} | {d['hit_rate']:.2%} | {d['mrr']:.3f} |")
    lines += ["", "## 逐用例", "",
              "| ID | 领域 | 命中 | 首位排名 | 相关条数 | 引用准确率 | 证据覆盖 |",
              "|---|---|---|---|---|---|---|"]
    for c in result["cases"]:
        lines.append(
            f"| {c['id']} | {c['domain']} | {'✅' if c['hit'] else '❌'} | "
            f"{c['rank'] or '-'} | {c['relevant_in_topk']} | "
            f"{c['precision_at_k']:.0%} | {c['evidence_coverage']:.0%} |")
    mp.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return jp, mp
...
def main() -> int:
    ...
    for c in result["cases"]:
        ...
        print(f"  [{mark}] {c['id']} rank={c['rank'] or '-'} "
              f"rel={c['relevant_in_topk']} cov={c['evidence_coverage']:.2f} "
              f"Q={c['question'][:38]}")
    print(f"报告: {jp}")
    print(f"报告: {mp}")
