...
def s_cover(prs):
    ...
    add_text(slide, Inches(1), Inches(3.3), Inches(11.3), Inches(0.6),
             [("招投标智能问答系统 · 5 大页面 26 项功能 · 浏览器自动化实测",
    ...
    add_text(slide, Inches(1.3), Inches(4.5), Inches(10.8), Inches(1.4), [
        ...
        ("结果: 26/26 项通过 · 发现并修复 4 个缺陷 (看板统计×2 / 图谱数据 / 图谱搜索)",
...
def s_summary(prs):
    ...
    rows = [
        ...
        ("文件解析页", "上传解析/文档列表/合规检查/人工确认", "✅ 通过",
         "txt 上传自动抽取8字段+入向量库; 15规则合规检查"),
        ("图谱可视化", "全图渲染/标的物搜索", "✅ 通过(补数据+修复)",
         "39节点/31关系 力导向图; 部分匹配搜索生效"),
...
def s_documents(prs):
    ...
    add_text(slide, Inches(8.3), Inches(1.2), Inches(4.7), Inches(5.8), [
        ...
        ("", {}),
        ("页面其他能力 (入口实测存在)", {"size": 15, "bold": True, "color": PRIMARY}),
        ("▸ 多家投标对比 / 智能工作流 / 报价计算 / 投标解析 / 围串标线索", {}),
        ("▸ 每份文档: 资格/废标/响应性检查 + 评分辅助表 + 一键生成标书章节", {}),
    ])
...
def s_bugs(prs):
    ...
    rows = [
        ...
        ("③", "图谱无数据可展示", "图谱可视化", "知识图谱仅 6 个手工孤立节点, 无 SubjectMatter/Supplier 子图", "新增 seed_knowledge_graph.py 从 30 条中标记录聚合生成 (幂等, --reset)", "39节点/31关系 渲染 ✅"),
...
def s_method(prs):
    ...
    rows = [
        ...
        ("功能覆盖", "26 项: 问答流式、多专家、拒答闸门、反馈、历史会话、模型切换、筛选、自动刷新、上传解析、合规检查、审计留痕、图谱渲染、图谱搜索、登录、脱敏、OCR 查阅等"),
    ...
    add_text(slide, Inches(0.4), Inches(5.9), Inches(12.5), Inches(1.0), [
        ("结论: 前端 5 大页面全部可用, 核心问答/检索/拒答/审计链路端到端跑通; 4 个缺陷已修复闭环",
